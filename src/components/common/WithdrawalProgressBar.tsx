...
const WithdrawalProgressBar = ({
  ...
  const getProgressBarColor = () => {
    ...
    if (currentStatus === 'Refunded') return 'bg-purple-500';
    if (currentStatus === 'Credited') return 'bg-green-500';
    return 'bg-blue-500';
  };
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {isCryptoWithdrawal && withdrawalRequest?.cryptoCoinType && (
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-900">{withdrawalRequest.cryptoCoinType}</span>
                </div>
              )}
              {isCryptoWithdrawal && withdrawalRequest?.cryptoNetworkType && (
                <div className="flex justify-between">
                  <span className="text-gray-600 uppercase tracking-wide">NETWORK</span>
                  <span className="font-bold text-gray-900">{withdrawalRequest.cryptoNetworkType}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          {isCryptoWithdrawal ? (
            <div className="space-y-2 text-sm">
              {withdrawalRequest?.cryptoWalletAddress && (
                <div>
              ...
              {withdrawalRequest?.cryptoNetworkType && (
                <div>
                  <span className="text-gray-600 uppercase tracking-wide">NETWORK:</span>
                  <span className="font-bold text-gray-900 ml-2">{withdrawalRequest.cryptoNetworkType}</span>
                </div>
              )}
              {withdrawalRequest?.cryptoCoinType && (
                <div>
                  <span className="text-gray-600 uppercase tracking-wide">COIN TYPE:</span>
                  <span className="font-bold text-gray-900 ml-2">{withdrawalRequest.cryptoCoinType}</span>
                </div>
              )}
            </div>
          ) : (
